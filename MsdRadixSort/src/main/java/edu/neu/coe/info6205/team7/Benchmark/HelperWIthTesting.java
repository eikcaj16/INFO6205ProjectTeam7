...
public class HelperWIthTesting<X extends Comparable<X>> implements Helper<X> {
  ...
  /**
   * Method to perform a stable swap using half-exchanges, i.e. between xs[i] and xs[j] such that
   * xs[j] is moved to index i, and xs[i] thru xs[j-1] are all moved up one. This type of swap is
  ...
  public X[] random(Class<X> clazz, Function<Random, X> f) {
    if (n <= 0) {
      throw new edu.neu.coe.huskySort.sort.BaseHelper.HelperException(
          "Helper.random: not initialized");
    }
   ...
   * @throws edu.neu.coe.huskySort.sort.BaseHelper.HelperException if n is inconsistent.
   */
  public void init(int n) {
    if (this.n == 0 || this.n == n) {
      ...
      throw new edu.neu.coe.huskySort.sort.BaseHelper.HelperException(
          "Helper: n is already set to a different value");
    }
  }
   ...
   * @param n           the number of elements expected to be sorted. The field n is mutable so can
   *                    be set after the constructor.
   * @param random      a random number generator.
   */
  public HelperWIthTesting(String description, int n, Random random) {
    this.n = n;
   ...
   */
  public HelperWIthTesting(String description, int n, long seed) {
   ...
   *                    be set after the constructor.
   */
  public HelperWIthTesting(String description, int n) {
   ...
   * @param description the description of this Helper (for humans).
   */
  public HelperWIthTesting(String description) {
    this(description, 0);
  }
  ...
  public static class HelperException extends RuntimeException {
    ...
    public HelperException(String message, Throwable cause, boolean enableSuppression,
